...
7. 使用[报告模板](assets/context-bootstrap-report-template.md)输出审核材料。对接入前项目，将已审核候选整理为后续 Harness 化输入；本 Skill 不执行 `init`。未获得写入授权时只报告建议；获得授权时也先生成草稿，维护者批准前不得写成已生效事实。
