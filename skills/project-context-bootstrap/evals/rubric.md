...
| 候选质量与准入动作 | 25 | 规则、契约、代码入口、README 人类导航、能力就绪、非准入和未确认项分类正确；就绪矩阵动态覆盖完整 Skill 集合并给出状态、Evidence、缺口和落点；没有真实 Knowledge 时不制造导航条目 |
...
| 输出与安全 | 10 | ready-for-review/partial/blocked 使用准确；完整接入建议包含 Distribution 且区分安装与前置条件就绪；不越权写入或泄露无关敏感内容 |
