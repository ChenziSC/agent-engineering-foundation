...
- 当前最小参考实现覆盖 Starter、带版本来源且可独立打包的 CLI、Specflow、Knowledge、可解释的 Context 路由、项目级兼容 Skill 运行时分发、Bootstrap/Continuous/Delivery 三级采用模板、本地 Git Change Gate、基于 Remote 与 Registry 的交付平台自动路由、可选 GitHub Actions Check/Workflow 只读证据、确定性契约检查和仓库静态检查；
