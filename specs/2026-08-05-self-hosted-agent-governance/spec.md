...
- [x] **AC-042** 全仓自然语言审计发现的事实源漂移已按最小范围修复：Knowledge 易变元数据只由 Registry 管理，迁移总览覆盖后续能力，能力问题图谱成熟度以各能力目录中的实现、测试和 Eval Evidence 为准，确定性参考实现与行为回放的成熟度语义不再重叠，正式回放清单不再跨文档手写复制，宠物资源具有公开维护者声明与当前文件摘要。
